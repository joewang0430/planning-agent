...
const KbSelector: React.FC = () => {
  const [kbList, setKbList] = useState<{ category: string; files: { name: string; type: string }[] }[]>([]);

  useEffect(() => {
    fetchKbList().then(setKbList).catch(console.error);
  }, []);
    ...
    <div className="flex flex-col gap-6 w-full h-full">
      {kbList.map(cat => (
        <div key={cat.category} className="mb-6">
          <div className="text-lg font-semibold text-blue-700 mb-2">{cat.category}</div>
          <div className="grid grid-cols-2 gap-4">
            {cat.files.map(file => (
              <KnowledgeBaseCard key={file.name} name={file.name} type="file" />
            ))}
          </div>
        </div>
      ))}
